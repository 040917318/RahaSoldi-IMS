    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-12 gap-4 lg:gap-8 h-auto lg:h-[calc(100vh-140px)]">
        ...
        <div className={`lg:col-span-8 flex-col bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl rounded-2xl sm:rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/20 dark:border-slate-700/50 overflow-hidden ${mobileTab === 'cart' ? 'hidden lg:flex' : 'flex'} h-auto min-h-[480px] lg:h-auto`}>
          ...
          {/* POS Footer Helper / Status Bar */}
          <div className="p-3 sm:p-4 bg-slate-50/80 dark:bg-slate-900/80 backdrop-blur-md border-t border-slate-200/80 dark:border-slate-700/80 flex-shrink-0 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
             <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                <span className="font-semibold text-slate-600 dark:text-slate-300">Terminal Ready</span>
             </div>
             <div className="flex items-center gap-1.5 font-medium text-[11px] text-slate-400">
                <Calculator className="w-3.5 h-3.5 opacity-60" />
                <span>Click any product to specify quantity & add to cart</span>
             </div>
          </div>
        </div>
      ...
      <AnimatePresence>
        {isDeferModalOpen && (
          <div className="fixed inset-0 z-[60] flex items-start justify-center p-3 sm:p-4 pt-3 sm:pt-8 pointer-events-auto">
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            <motion.div 
              ...
              className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl shadow-[0_32px_64px_-12px_rgba(0,0,0,0.5)] w-full max-w-lg max-h-[calc(100vh-3rem)] flex flex-col overflow-hidden relative z-10"
            >
              <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-start shrink-0">
                <div>
                  ...
                  <h3 className="text-lg sm:text-2xl font-black text-slate-800 dark:text-white tracking-tight">Initialize Deferral</h3>
                  <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">Record transaction details for outstanding balance recovery.</p>
                </div>
                <motion.button 
              ...
              <div className="p-4 sm:p-5 space-y-3 sm:space-y-4 overflow-y-auto flex-1 custom-scrollbar">
                <div className="space-y-1.5">
                  ...
                  <div className="relative group">
                    ...
                    <input
                      ...
                      className="w-full pl-11 sm:pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-xl sm:rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-all font-bold text-xs sm:text-sm text-slate-800 dark:text-white placeholder-slate-300"
                      value={customerName}
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full p-3 sm:p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-xl sm:rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-all min-h-[70px] font-medium text-xs sm:text-sm text-slate-800 dark:text-white placeholder-slate-300"
                    value={notes}
                    onChange={(e) => setNotes(e.target.value)}
                  />
                </div>

                <div className="bg-indigo-600/5 p-3.5 rounded-xl sm:rounded-2xl border border-indigo-600/10 flex justify-between items-center">
                  <div className="flex flex-col">
                    <span className="text-[9px] sm:text-[10px] font-black text-indigo-500 uppercase tracking-widest">Calculated Overdue</span>
                    <span className="text-lg sm:text-2xl font-black text-indigo-600 dark:text-indigo-400 tracking-tighter">
                      {currencySymbol}{cartTotal.toFixed(2)}
                    </span>
                  </div>
                  <Calculator className="w-7 h-7 sm:w-8 sm:h-8 text-indigo-300 dark:text-indigo-800" />
                </div>
              </div>

              <div className="p-4 sm:p-5 bg-slate-50/50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800 flex gap-3 shrink-0">
                <motion.button
                  whileHover={{ backgroundColor: 'rgba(255,255,255,1)' }}
                  onClick={() => setIsDeferModalOpen(false)}
                  className="flex-1 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold rounded-xl transition-all text-xs sm:text-sm"
                >
                  Discard
                </motion.button>
                <motion.button
                  ...
                  className="flex-2 py-2.5 bg-indigo-600 text-white font-black rounded-xl shadow-lg shadow-indigo-600/20 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all uppercase tracking-widest text-xs sm:text-sm"
                >
                  Finalize Record
                </motion.button>
              </div>
        ...
        {isAddToCartModalOpen && selectedProduct && (
          <div className="fixed inset-0 z-[60] flex items-start justify-center p-3 sm:p-4 pt-3 sm:pt-8 pointer-events-auto">
            ...
            <motion.div 
              ...
              className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] w-full max-w-sm sm:max-w-md border border-slate-200 dark:border-slate-800 overflow-hidden relative z-10 max-h-[calc(100vh-3rem)] flex flex-col"
            >
              {/* Header */}
              <div className="p-3.5 sm:p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50 shrink-0">
                <div className="flex items-center gap-2.5">
                  <div className="p-2 bg-indigo-600/10 rounded-xl text-indigo-600 dark:text-indigo-400">
                    <Package className="w-4 h-4 sm:w-5 sm:h-5" />
                  </div>
                  <div>
                    <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-indigo-500 block">
                      Add To Order
                    </span>
                    ...
                    <div className="flex items-center gap-1.5 text-[11px] text-slate-500 dark:text-slate-400 font-medium mt-0.5">
                      <span>{currencySymbol}{selectedProduct.salesPrice.toFixed(2)}</span>
                      <span>•</span>
                      <span className={selectedProduct.quantity > selectedProduct.lowStockThreshold ? 'text-emerald-600 dark:text-emerald-400 font-semibold' : 'text-rose-500 font-semibold'}>
                ...
                <motion.button 
                  ...
                  className="p-1 hover:bg-slate-200/50 dark:hover:bg-slate-800 rounded-lg transition-colors"
                >
              ...
              {/* Modal Body */}
              <div className="p-3 sm:p-4 space-y-2.5 sm:space-y-3 overflow-y-auto flex-1 custom-scrollbar">
                {itemInCartTotal(cart, selectedProduct.id) > 0 && (
                  <div className="bg-amber-500/10 border border-amber-500/20 text-amber-700 dark:text-amber-400 text-[11px] sm:text-xs font-bold p-2 rounded-xl flex items-center justify-between">
                    <span>In Cart Currently:</span>
                    <span className="font-mono text-xs">{itemInCartTotal(cart, selectedProduct.id)} units</span>
                  </div>
                )}
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center p-1 bg-slate-100 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                    <motion.button 
                      ...
                      className="p-1.5 sm:p-2 text-slate-600 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-700 rounded-lg transition-all"
                    >
                    ...
                    <input 
                      ...
                      className="w-full text-center font-black text-base sm:text-lg focus:outline-none bg-transparent text-slate-900 dark:text-slate-100"
                      value={qtyInput}
                ...
                <div className="bg-indigo-600/5 dark:bg-indigo-600/10 p-2.5 sm:p-3 rounded-xl border border-indigo-600/10 flex justify-between items-center">
                  <div>
                    <span className="text-[9px] font-black uppercase text-indigo-500 tracking-wider block">Item Subtotal</span>
                  ...
                  <ShoppingCart className="w-5 h-5 sm:w-6 sm:h-6 text-indigo-400/40" />
                </div>
              </div>

              {/* Modal Actions */}
              <div className="p-3 sm:p-4 bg-slate-50/50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800 flex gap-2 shrink-0">
                <button
                  type="button"
                  onClick={() => {
                    setIsAddToCartModalOpen(false);
                  ...
                  className="px-3.5 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold rounded-xl hover:bg-slate-200 text-xs uppercase tracking-wider transition-all"
                >
                  Cancel
                </button>
                <motion.button 
                  ...
                  className="flex-1 py-2.5 bg-indigo-600 text-white font-black rounded-xl hover:bg-indigo-700 shadow-md shadow-indigo-600/20 transition-all flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed uppercase tracking-wider text-xs"
                >
